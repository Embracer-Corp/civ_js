...
const room = {
  players: [],
  nickname: null,
  draw: function(ctx = canvasContext, ctxWidth = canvas.width, ctxHeight = canvas.height, timePass) {
    ...
    // let y_offset = this.nickname == null? 0 : 1;
    for(let i = 0; i < this.players.length; i++)
    {
      ctx.fillStyle = this.players[i] == this.nickname?"#B3370B":"#333";
      ctx.fillText(this.players[i], 30, 160 + i*30)
      // if(this.players[i] == this.nickname) {
      //   ctx.fillText(this.players[i], 30, 160)
      // } else {
      //   ctx.fillText(this.players[i], 30, 160 + (y_offset++)*30)
      // }
    }
  ...
  init: function() {
    ...
    socket.on("players_info", (players) => {
      console.log(`[players_info] players: ${players}\r\nYou nick: ${this.nickname}`)
